...
const MediaContentCollage: React.FC<any> = ({
  backgroundColor,
  content,
  enableCTA,
  link,
  ...
  return (
    <Gutter className={classes.wrap} left right>
      <BackgroundColor color={backgroundColor} className={classes.bg}>
        ...
        <GridContainer>
          <Grid>
            <Cell cols={10} start={2} startM={1}>
              <RichText content={content} className={classes.content} />
              {enableCTA && (
                <Link {...link} className={classes.cta}>
                  <Button color={backgroundColor} label={link.label} css={buttonCSS} />
                </Link>
              )}
            </Cell>
          </Grid>
