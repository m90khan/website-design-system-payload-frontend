...
import { StatisticsType } from '@/lib/types/layout';

const Statistics: React.FC<StatisticsType> = ({
  topOverlap,
  bottomOverlap,
  stats,
  backgroundColor,
  ...
  return (
    <div className={classes.overflow}>
      ...
      >
        ...
        <ul className={[classes.statistics].filter(Boolean).join(' ')}>
          {stats?.map((stat, i) => (
            <li key={i} className={classes.stat}>
              <Parallax className={classes[`stat${i + 1}`]} yDistance={50 * (i + 1)}>
                <Statistic {...stat} />
              </Parallax>
            </li>
          ))}
