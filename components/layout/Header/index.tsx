...
import { MegaMenuType, SocialMediaType } from '@/lib/types/common';
...
const Header: React.FC<Props> = ({ megaMenu, socialMedia }) => {
  ...
  return (
    <header className={classes.header}>
      <NextLink href='/' scroll={false} className={classes.logo}>
          ...
          <Grid className={classes.navLinks}>
            <Cell cols={8} htmlElement='nav'>
              {megaMenu?.nav?.map(({ link }, i) => (
                <h3 style={i === 0 ? { marginTop: 0 } : undefined} key={link.label}>
